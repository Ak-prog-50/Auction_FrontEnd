...
const Redeem = () => {
  ...
  const { addrs, chainId, highestBidAmount, highestBidder } = useContext(AuctionContext) as IAuctionContext
  const auctionAddress = chainId ? addrs[chainId] : undefined;

  const { redeem, fetchingRedeem, loadingRedeem } = useAuctionCalls(addrs, chainId);
    ...
    <button
      ...
      onClick={async () => {
        if (highestBidderChecksum === accountChecksum) {
          await redeemExecute(redeem, auctionAddress, highestBidAmount, dispatch)
        } else {
